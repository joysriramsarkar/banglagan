...
export default function SongSuggestions() {
  ...
  React.useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        const output = await suggestSongsBasedOnHistory({ searchHistory: historyArray.join(',') });
    ...
    try {
      ...
      if (storedHistory) {
        ...
        if (Array.isArray(parsed)) {
          return parsed.filter(item => item && String(item).trim() !== '').length;
        }
        console.warn('Search history in localStorage is not an array, resetting.');
        localStorage.setItem('searchHistory', JSON.stringify([]));
        return 0;
  ...
  const shouldRenderSuggestions = initialHistory === null ? false : initialHistory > 0 || loading || error;
  ...
  return (
    <Card className="bg-secondary/50 border-accent shadow-md">
      ...
      <CardContent>
        ...
        {!loading && !error && (!enrichedSuggestions || enrichedSuggestions.length === 0) && initialHistory > 0 && (
         ...
         {hasAttemptedFetch && initialHistory === 0 && (
            <p className="text-muted-foreground text-sm">এখানে গানের সুপারিশ দেখতে, প্রথমে কিছু গান অনুসন্ধান করুন।</p>
         )}
      </CardContent>
    </Card>
  );
